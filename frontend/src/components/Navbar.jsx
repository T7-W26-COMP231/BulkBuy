...
const GTA_CITIES = [
  "Toronto", "Scarborough", "Mississauga", "Brampton", "Markham",
  "Vaughan", "Richmond Hill", "Oakville", "Burlington", "Pickering",
  "Ajax", "Whitby", "Oshawa", "Milton", "Newmarket", "Aurora",
];
...
export default function Navbar({ detectedCity, onCityChange, locationLabel, onSearch }) {
  ...
  const handleCitySelect = (city) => {
    ...
    setOpen(false);
    setMobileOpen(false); // close mobile menu after city pick
  ...
  const handleSignOut = async () => {
    try {
      await signOut?.();
      try {
        showToast(<div style={{ padding: 12 }}><strong>Signed out</strong></div>, {
          value: "TR", IsToStack: false, duration: 2500, toastName: "Signed out",
        });
      } catch (tErr) { console.error("showToast error:", tErr); }
    } catch (err) {
      ...
      try {
        showToast(
          ...
          { value: "TR", IsToStack: false, duration: 4000, toastName: "Sign out failed" }
        );
      } catch (tErr) { console.error("showToast error:", tErr); }
    }
  };
  ...
  function ToastAuthWrapper({ toastcontrols }) {
    ...
    const handleSignIn = async (payload) => {
      if (!signInHandler) {
        globalShow(<div style={{ padding: 12 }}><strong>Auth unavailable</strong></div>, { value: "TR", IsToStack: false });
      ...
      try {
        ...
        try {
          toastcontrols?.update?.({ content: <div style={{ padding: 12 }}><strong>Sign in error</strong><div style={{ marginTop: 6 }}>{err?.message || "Unknown error"}</div></div> });
        } catch (uErr) { console.error("toastcontrols.update error:", uErr); }
        globalShow(<div style={{ padding: 12 }}><strong>Sign in error</strong><div style={{ marginTop: 6 }}>{err?.message || "Unknown error"}</div></div>, { value: "TR", IsToStack: false, duration: 4000 });
    ...
    const handleSignUp = async (payload) => {
      ...
      try {
        ...
        try {
          toastcontrols?.update?.({ content: <div style={{ padding: 12 }}><strong>Registration error</strong><div style={{ marginTop: 6 }}>{err?.message || "Unknown error"}</div></div> });
        } catch (uErr) { console.error("toastcontrols.update error:", uErr); }
        globalShow(<div style={{ padding: 12 }}><strong>Registration error</strong><div style={{ marginTop: 6 }}>{err?.message || "Unknown error"}</div></div>, { value: "TR", IsToStack: false, duration: 4000 });
        return { ok: false, error: err?.message || "Registration error" };
      }
    };

    return (
      <div style={{ minWidth: 420, position: "relative", textAlign: "center", borderRight: "1px solid black", borderRadius: "5px" }}>
  ...
  const openAuthToast = () => {
    try {
      showToast(<ToastAuthWrapper />, {
        value: "HVC", IsToStack: true, toastName: "Auth",
        AllowedMultiple: false, IsToStick: true, blurBg: true, duration: null, animate: "TR",
      });
    } catch (err) { console.error("openAuthToast showToast error:", err); }
  };
    ...
    <header className="border-b border-neutral-light bg-white px-4 py-3 md:px-10 lg:px-20">
                ...
                <div className="max-h-64 overflow-y-auto py-1">
                  {GTA_CITIES.map((city) => (
                    <button key={city} type="button" onClick={() => handleCitySelect(city)}
                      className={`flex w-full items-center gap-2 px-4 py-2 text-left text-sm transition-colors hover:bg-gray-50 ${selected === city ? "bg-blue-50 font-semibold text-primary" : "text-gray-700"}`}>
        ...
        <div className="hidden flex-1 items-center justify-end gap-4 md:flex">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={profileRef}>
              {user ? (
                <>
                  <button type="button" onClick={() => setProfileOpen((s) => !s)}
                      ...
                      <div className="border-b border-gray-100 px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {user?.role && <div className="mt-1 text-xs text-text-muted">{user.role}</div>}
                          </div>
                        </div>
                      </div>
                      <div className="py-1">
                ...
                <button type="button" onClick={openAuthToast}
      ...
      {mobileOpen && (
        <div className="mt-3 flex flex-col gap-3 border-t border-gray-100 pt-3 md:hidden">
          ...
          {/* City picker */}
          <div>
            <p className="mb-1 px-1 text-xs font-semibold text-text-muted">Location</p>
          ...
          <div className="flex flex-col">
            ...
            <Link to="/orders" className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-gray-700 hover:bg-gray-50" onClick={() => setMobileOpen(false)}>
              <span className="material-symbols-outlined text-base">receipt_long</span> Orders
            </Link>
            ...
            {user ? (
              <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2">
                  <img src={avatarSrc} alt="avatar" className="h-8 w-8 rounded-full object-cover border border-primary" />
                  <div>
                    <div className="text-sm font-semibold">{displayName || "User"}</div>
              ...
              <button type="button" onClick={() => { setMobileOpen(false); openAuthToast(); }}
