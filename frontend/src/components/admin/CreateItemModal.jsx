...
const STATUS_OPTIONS = ["draft", "active", "suspended"];
...
const INITIAL = {
    ...
    shortDescription: "",
    description: "",
    ops_regions: [],          // ✅ was ops_region (string) — model uses ops_regions (array)
    status: "draft",
    published: false,
    brand: { name: "" },
    tags: "",
    ...
    taxClass: "",
    seller: { name: "" },    // ✅ added — was never collected
};

export default function CreateItemModal({ productId, existingItemIds = [], onClose, onCreated }) {
    ...
    // ✅ toggle a region in/out of the ops_regions array
    const toggleRegion = (region) => {
        setForm((prev) => {
            const already = prev.ops_regions.includes(region);
            return {
                ...prev,
                ops_regions: already
        ...
        try {
            ...
            const price = form.price[0].list
                ? [{
                    ...
                    effectiveFrom: form.price[0].effectiveFrom
                        ? new Date(form.price[0].effectiveFrom)
                        : null,
                    effectiveTo: form.price[0].effectiveTo
                        ? new Date(form.price[0].effectiveTo)
                        : null,
                }]
                : [];
            ...
            const payload = {
                ...
                ops_regions: form.ops_regions,               // ✅ fixed: array, correct key
                status: form.status,
                published: form.published,
                brand: { name: form.brand.name.trim() },
                seller: { name: form.seller.name.trim() },  // ✅ added
                tags,
                inventory: {
            ...
            const iid = String(created._id);

            // 2. link to product — best-effort, never blocks success
            if (productId) {
                try {
                    const existingRefs = existingItemIds.map((id) => ({ itemId: id }));
                    await api.patch(`/prdts/${productId}`, {
                        items: [...existingRefs, { itemId: iid }],
                    });
                } catch (linkErr) {
                    console.warn("Item created but failed to link to product:", linkErr.message);
                }
            }

            onCreated({ ...created, _id: iid });
            onClose();
        } catch (err) {
            setErrors({ submit: err?.response?.data?.message || "Failed to create item" });
        } finally {
            setSaving(false);
        }
    };
        ...
        >
            <div className="w-full max-w-xl rounded-2xl bg-white shadow-xl border border-neutral-light overflow-hidden">
                ...
                <div className="flex items-center justify-between border-b border-neutral-light px-6 py-4">
                    <div>
                        <h2 className="text-lg font-bold text-text-main">Create item</h2>
                        <p className="text-xs text-text-muted mt-0.5">
                            {productId ? "Item will be linked to the selected product" : "Standalone item"}
                        </p>
                    </div>
                    ...
                    {section === "basic" && (
                        <>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="mb-2 block text-sm font-semibold text-text-main">Seller name</label>
                                    <input type="text" placeholder="e.g. Acme Corp"
                                        value={form.seller.name} onChange={(e) => setNested("seller", "name", e.target.value)}
                                        className="w-full rounded-xl border border-neutral-light bg-white px-4 py-3 text-sm outline-none focus:border-primary" />
                                </div>
                            </div>
                            ...
                            {/* ✅ ops_regions — multi-select chips instead of single dropdown */}
                            <div>
                                <label className="mb-2 block text-sm font-semibold text-text-main">Ops regions</label>
                                <div className="flex flex-wrap gap-2">
                                    {REGIONS.map((r) => {
                                        const active = form.ops_regions.includes(r);
                                        return (
                                            <button key={r} type="button" onClick={() => toggleRegion(r)}
                                                className={`rounded-lg border px-3 py-1.5 text-xs font-medium transition ${active
                                                    ...
                                                    : "border-neutral-light text-text-muted hover:border-primary/50"}`}>
                                                {r}
                                            </button>
                                        );
                                    })}
                                </div>
                                {form.ops_regions.length > 0 && (
                                    <p className="mt-1 text-xs text-text-muted">
                                        Selected: {form.ops_regions.join(", ")}
                                    </p>
                                )}
                            </div>
                            ...
                            <div>
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input type="number" min="0" step="0.01" placeholder="0.00"
                                                value={form.price[0].list}
                                                onChange={(e) => setPrice("list", e.target.value)}
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input type="number" min="0" step="0.01" placeholder="Optional"
                                                value={form.price[0].sale}
                                                onChange={(e) => setPrice("sale", e.target.value)}
                                                className="w-full rounded-xl border border-neutral-light bg-white py-3 pl-8 pr-4 text-sm outline-none focus:border-primary" />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mt-3 grid grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        <select value={form.price[0].currency}
                                    ...
                                    <div>
                                        ...
                                        <input type="date"
                                            value={form.price[0].effectiveFrom}
                                    ...
                                    <div>
                                        ...
                                        <input type="date"
                                            value={form.price[0].effectiveTo}
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="number" min="0" step="0.01" placeholder="0"
                                            ...
                                            onChange={(e) => setNested("weight", "value", e.target.value)}
                                            className="w-full rounded-xl border border-neutral-light bg-white px-4 py-3 text-sm outline-none focus:border-primary" />
                                    </div>
                                    <div>
                                        <label className="mb-2 block text-xs font-semibold text-text-muted">Unit</label>
                                        <select value={form.weight.unit}
                                            onChange={(e) => setNested("weight", "unit", e.target.value)}
                                            className="w-full rounded-xl border border-neutral-light bg-white px-4 py-3 text-sm outline-none focus:border-primary">
                                ...
                                <div className="grid grid-cols-4 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <select value={form.dimensions.unit}
                                            onChange={(e) => setNested("dimensions", "unit", e.target.value)}
                                            className="w-full rounded-xl border border-neutral-light bg-white px-3 py-3 text-sm outline-none focus:border-primary">
                                ...
                                <div className="flex flex-col gap-3">
                                    <div>
                                        ...
                                        <input type="text" placeholder="e.g. standard, express"
                                            value={form.shipping.class}
                                            onChange={(e) => setShipping("class", e.target.value)}
                                            className="w-full rounded-xl border border-neutral-light bg-white px-4 py-3 text-sm outline-none focus:border-primary" />
                                    </div>
                                    <div>
                                        ...
                                        <input type="text" placeholder="e.g. 50, 70, 100"
                                            value={form.shipping.freightClass}
                                            onChange={(e) => setShipping("freightClass", e.target.value)}
                                            className="w-full rounded-xl border border-neutral-light bg-white px-4 py-3 text-sm outline-none focus:border-primary" />
                                    </div>
                                    <div>
                                        ...
                                        <input type="text" placeholder="e.g. Toronto, ON"
                                            ...
                                            onChange={(e) => setShipping("shipsFrom", e.target.value)}
                                            className="w-full rounded-xl border border-neutral-light bg-white px-4 py-3 text-sm outline-none focus:border-primary" />
                                    </div>
                                </div>
                            </div>
